...
- **~10× fewer tokens and ~3× faster** on multi-hop tasks (the [numbers above](#the-numbers)) - intermediate state is piped tool-to-tool, never re-billed through the model.
...
- **Projection defaults to `OPAQUE`.** Tool outputs are hidden from the model
  ...
  that output's projection. See [Projection Types](./docs/concepts/projection-types.md).
